...
The onboarding funnel is `sign-up -> verify-email -> onboarding wizard -> subscribe -> Stripe checkout (7-day trial) -> /app`. Most of that is handled in code, but a few things live in the **Neon Console** and **Resend**. Keep this page in sync whenever you change either service.
