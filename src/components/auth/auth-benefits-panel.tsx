import Image from "next/image";
import { ArrowRight, Map, Mail, Users } from "lucide-react";
import { HOME_CHAPTERS } from "@/lib/marketing/images";
import {
  AUTH_BENEFITS,
  AUTH_STATS,
  PRODUCT_DESCRIPTION,
  PRODUCT_HEADLINE,
  PRODUCT_SIGNIN_HEADLINE,
} from "@/lib/marketing/copy";
import { startFreeTrialLabel } from "@/lib/trial";
import { AuthTrustStrip } from "@/components/auth/auth-trust-strip";
import { AuthTransitionLink } from "@/components/auth/auth-transition-link";

const BENEFIT_ICONS = [Map, Users, Mail] as const;

type Props = {
  variant?: "signup" | "signin" | "verify";
  signUpHref?: string;
};

export function AuthBenefitsPanel({
  variant = "signup",
  signUpHref = "/auth/sign-up",
}: Props) {
  const hero = HOME_CHAPTERS.map;
  const isSignIn = variant === "signin";

  const headline =
    variant === "signin"
      ? PRODUCT_SIGNIN_HEADLINE
      : variant === "verify"
        ? "You're almost in."
        : PRODUCT_HEADLINE;

  const subline =
    variant === "signin"
      ? "Live planning intelligence for construction, property and planning teams."
      : variant === "verify"
        ? "Verify your email to unlock map search, enrichment and branded outreach."
        : PRODUCT_DESCRIPTION;

  return (
    <div className="relative flex h-full min-h-0 flex-col overflow-y-auto bg-zinc-950 p-8 lg:p-10 xl:p-12">
      <Image
        src={hero.src}
        alt={hero.alt}
        fill
        priority
        className="object-cover opacity-40"
        sizes="50vw"
      />
      <div
        className="pointer-events-none absolute inset-0 bg-gradient-to-br from-zinc-950/95 via-zinc-950/75 to-zinc-950/90"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute inset-0 opacity-[0.04]"
        style={{
          backgroundImage:
            "radial-gradient(circle at 1px 1px, white 1px, transparent 0)",
          backgroundSize: "24px 24px",
        }}
        aria-hidden
      />

      <div className="relative z-10 flex min-h-0 flex-1 flex-col">
        <div>
          <p className="editorial-chapter-label text-brand-light/80">
            Plott · Planning intelligence
          </p>
          <h2 className="mt-4 max-w-md font-[family-name:var(--font-display)] text-[clamp(26px,3.2vw,40px)] font-normal leading-[1.12] tracking-tight text-white">
            {headline}
          </h2>
          <p className="mt-3 max-w-sm text-[15px] leading-relaxed text-zinc-300">
            {subline}
          </p>
        </div>

        <div
          className={
            isSignIn
              ? "mt-8 flex flex-1 flex-col justify-end gap-6"
              : "mt-8 space-y-6"
          }
        >
          {!isSignIn ? (
            <>
              <ul className="space-y-4">
                {AUTH_BENEFITS.map(({ title, description }, index) => {
                  const Icon = BENEFIT_ICONS[index] ?? Map;
                  return (
                    <li key={title} className="flex gap-3">
                      <span className="mt-0.5 flex h-8 w-8 shrink-0 items-center justify-center rounded-full border border-brand-light/25 bg-brand/15">
                        <Icon className="h-4 w-4 text-brand-light" aria-hidden />
                      </span>
                      <div>
                        <p className="text-sm font-medium text-white">{title}</p>
                        <p className="mt-0.5 text-sm leading-relaxed text-zinc-400">
                          {description}
                        </p>
                      </div>
                    </li>
                  );
                })}
              </ul>

              <div className="grid grid-cols-3 gap-4 border-t border-white/10 pt-6">
                {AUTH_STATS.map(({ value, label }) => (
                  <div key={label}>
                    <p className="font-[family-name:var(--font-display)] text-2xl text-white">
                      {value}
                    </p>
                    <p className="mt-1 text-[11px] leading-snug text-zinc-400">
                      {label}
                    </p>
                  </div>
                ))}
              </div>
            </>
          ) : null}

          {isSignIn ? (
            <div className="rounded-xl border border-white/10 bg-white/5 p-5 backdrop-blur-sm">
              <p className="text-sm font-medium text-white">New to Plott?</p>
              <p className="mt-1 text-sm text-zinc-400">
                Start a free trial — map applications, enrich contacts and send
                outreach in minutes.
              </p>
              <AuthTransitionLink
                href={signUpHref}
                direction="signup"
                className="mt-4 inline-flex items-center gap-2 rounded-full bg-white/90 px-5 py-2.5 text-[13px] font-semibold text-zinc-900 transition hover:bg-white"
              >
                {startFreeTrialLabel()}
                <ArrowRight className="h-3.5 w-3.5" aria-hidden />
              </AuthTransitionLink>
            </div>
          ) : null}

          {!isSignIn ? (
            <AuthTrustStrip variant="dark" className="justify-start pb-1" />
          ) : null}
        </div>
      </div>
    </div>
  );
}
