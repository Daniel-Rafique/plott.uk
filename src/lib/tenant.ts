...
/** Thrown when Neon Auth re-issued an id for an email that already has app data. */
...
function isAutoGeneratedWorkspaceName(name: string): boolean {
  return name.endsWith("'s Workspace") || name === "My Workspace";
}

/** True when a stale Prisma user row can be safely replaced after Neon re-registration. */
async function isAbandonedSignup(userId: string): Promise<boolean> {
  const [letterCount, memberships] = await Promise.all([
  ...
  return (
    !company.onboardingCompletedAt && isAutoGeneratedWorkspaceName(company.name)
  );
}
...
async function updateUserFromSession(
  userId: string,
  session: SessionUser,
...
export async function upsertUserFromSession(
  ...
  const emailVerifiedAt = nowIfVerified ?? null;

  const existingById = await prisma.user.findUnique({
    where: { id: session.id },
  });
  if (existingById) {
    return updateUserFromSession(session.id, session, nowIfVerified);
  }
  ...
  if (normalizedEmail) {
    ...
    if (existingByEmail && existingByEmail.id !== session.id) {
      if (await isAbandonedSignup(existingByEmail.id)) {
        logger.info(
          { email: normalizedEmail, staleId: existingByEmail.id, newId: session.id },
          "tenant_reconcile_abandoned_signup",
        );
        await prisma.user.delete({ where: { id: existingByEmail.id } });
      } else {
        logger.error(
          { email: normalizedEmail, staleId: existingByEmail.id, newId: session.id },
          "tenant_stale_auth_user_id_mismatch",
        );
        throw new StaleAuthUserError(normalizedEmail);
      }
    }
  }

  return createUserFromSession(session, emailVerifiedAt);
}
